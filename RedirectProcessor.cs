...
namespace Sitecore.SharedSource.RedirectManager
{
    ...
    public static class RedirectProcessor
    {
        ...
        /// </summary>
        private const string ItemsCacheKey = "Sitecore.RedirectManager:Items " + Items.ItemIDs.RedirectsFolderItem;

        /// <summary>
        ///   Cache key for sections redirects
        /// </summary>
        private const string SectionsCacheKey = "Sitecore.RedirectManager:Sections " + Items.ItemIDs.RedirectsFolderItem;

        /// <summary>
        ///   Cache key for RegEx redirects
        /// </summary>
        private const string RegExCacheKey = "Sitecore.RedirectManager:RegEx " + Items.ItemIDs.RedirectsFolderItem;
        ...
        public static void CreateListOfRedirects()
        {
            lock (syncObject)
            {
                var sw = new Stopwatch();
                sw.Start();
                var redirectForlder = Factory.GetDatabase(Configuration.Database).GetItem(Items.ItemIDs.RedirectsFolderItem);
                if (redirectForlder == null)
                {
                    LogManager.WriteError(string.Format("Forlder with redirects \"{0}\" not found", Items.ItemIDs.RedirectsFolderItem));
                    return;
                }
          ...
          string targetItemId, string baseSectionUrl, string targetSectionUrl, ID itemId, int redirectCode)
        {
            ...
            {
                list.AddRange(
                    sectionItem.Axes.GetDescendants().Where(CheckPresentation)
                    .Select(UrlNormalizer.GetItemUrl)
                            .Select(targetUrl => new
                            {
                                targetUrl,
                                baseUrl = string.Format("{0}{1}", baseSectionUrl, targetUrl)
                            })
                            .Select(@t => new RedirectItem
                                {
                                    ItemId = itemId,
                                    Target = @t.targetUrl,
                                    Base = @t.baseUrl,
